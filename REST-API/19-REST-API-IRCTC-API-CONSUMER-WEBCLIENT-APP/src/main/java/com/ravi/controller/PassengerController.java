...
public class PassengerController {
	...
	public String bookTicket(Passenger passenger, Model model) {
		Mono<Ticket> ticket = passengerService.bookTicket(passenger);
	...
	public String getTickets(Model model) {
		Mono<Ticket[]> tickets = passengerService.getAllTickets();
	...
	public String searchTicket(@RequestParam("id") Integer id, Model model) {
		Mono<Ticket> ticket = passengerService.getTicket(id);
		if (Mono.empty() != null) {
			model.addAttribute("ticket", ticket);
			return "search-ticket";
		} else {
